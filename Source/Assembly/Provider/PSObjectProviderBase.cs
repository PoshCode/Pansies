...
namespace PoshCode.Pansies
{
    ...
    public abstract class PSObjectProviderBase : ContainerCmdletProvider, IContentCmdletProvider
    {
        private PSObjectDriveInfo ObjectDriveInfo
        {
            get { return PSDriveInfo as PSObjectDriveInfo; }
        }
        ...
        private IEnumerable<NamedObject> GetProperties(string path)
        {
            if (CurrentObject == null)
            {
                yield break;
            ...
            if (string.IsNullOrEmpty(path))
            {
                ...
                {
                    yield return new NamedObject(prop.Name, prop.Value);
                }
            }
            else
            {
        ...
        private PSObjectProviderContent GetContent(string path)
        {
            if (CurrentObject != null)
            {
                return new PSObjectProviderContent(CurrentObject, path);                
            }
